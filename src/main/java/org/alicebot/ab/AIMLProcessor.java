...
public class AIMLProcessor {
    private static final Logger log = LoggerFactory
            .getLogger(org.alicebot.ab.AIMLProcessor.class);
    /**
    ...
    private static void categoryProcessor(Node n, ArrayList<Category> categories, String topic, String aimlFile, String language) {
        ...
        if (language.equals("JP") || language.equals("jp")) {
            String morphPattern = org.alicebot.ab.JapaneseTokenizer.morphSentence(pattern);
            log.info("<pattern>" + pattern + "</pattern> --> <pattern>" + morphPattern + "</pattern>");
            pattern = morphPattern;
            String morphThatPattern = org.alicebot.ab.JapaneseTokenizer.morphSentence(that);
     ...
     * @return response from remote service or string indicating failure.
     */
    private static String search(Node node, ParseState ps) {
        final HashSet<String> attributeNames = Utilities.stringSet("botid", "host");
        final String content = evalTagContent(node, ps, attributeNames);

        return SearchContext.process(ps.chatSession, content);
    ...
    private static String recursEval(Node node, ParseState ps) {
        try {
            ...
            else if (nodeName.equals("search"))
                return search(node, ps);
